...
export function useServiceDimensions(serviceName: string | null) {
  const [dimensions, setDimensions] = useState<ServiceDimensionsResponse | null>(null)
...
export function useChecklistValidation(serviceName: string | null) {
  const [validation, setValidation] = useState<ChecklistValidateResponse | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  ...
  const validate = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to validate checklist'
      setError(message)
      return null
    } finally {
      setLoading(false)
