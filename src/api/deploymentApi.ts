import type { ServiceStatus, DeployParams, DeployJob } from '../types/deploymentTypes'

const DEPLOYMENT_API = import.meta.env.VITE_DEPLOYMENT_API_URL ?? 'http://localhost:8004'
...
export async function rollbackDeployment(jobId: string): Promise<DeployJob> {
  const response = await fetch(`${DEPLOYMENT_API}/api/v1/deployments/${jobId}/rollback`, {
    method: 'POST',
  })
  return handleResponse<DeployJob>(response)
}
