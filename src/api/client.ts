...
export async function getActiveConfig(
  ...
  return fetchJson<ConfigReadResponse>(`/config-store/${domain}`);
}
...
export async function writeConfig(
  ...
  return fetchJson<ConfigVersionResponse>(`/config-store/${domain}`, {
    method: "POST",
...
export async function listConfigVersions(
  ...
  return fetchJson<ConfigVersionEntry[]>(`/config-store/${domain}/versions`);
}
...
export async function diffConfigVersions(
  ...
  return fetchJson<ConfigDiffResponse>(
    `/config-store/${domain}/versions/${ts1}/diff/${ts2}`,
  );
}
...
export async function rollbackConfig(
  ...
  return fetchJson<ConfigVersionResponse>(
    `/config-store/${domain}/rollback/${timestamp}`,
    {
      method: "POST",
      body: JSON.stringify({}),
    },
