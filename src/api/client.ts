...
// Services
export async function getServices(): Promise<{ services: Service[]; count: number }> {
...
export async function retryFailedShards(
  ...
  try {
    const response = await fetch(`${API_BASE}/deployments/${id}/retry-failed${query ? `?${query}` : ''}`, {
      method: 'POST',
...
export async function cancelShard(deploymentId: string, shardId: string): Promise<CancelShardResult> {
  return fetchJson(`/deployments/${deploymentId}/shards/${shardId}/cancel`, { method: 'POST' })
}
...
export async function deleteDeployment(id: string): Promise<{ deployment_id: string; deleted: boolean; message: string }> {
...
export async function listChecklists(): Promise<{ checklists: ChecklistSummary[]; count: number }> {
...
export async function getVenueFilters(category: string, venue: string): Promise<VenueFiltersResponse> {
  const searchParams = new URLSearchParams()
...
export async function getServiceCategories(serviceName: string): Promise<ServiceCategoriesResponse> {
...
export async function clearDataStatusCache(): Promise<{ status: string; entries_cleared: number }> {
...
export async function getInstrumentAvailability(params: {
  ...
  return fetchJson(`/data-status/instrument-availability?${searchParams.toString()}`)
}
