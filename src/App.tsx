...
import { ServiceUrlSync, isHomeShellPath } from "./components/ServiceUrlSync";
...
// Guards the `*` catch-all: it's shared by the per-service home shell (`/home`,
// `/service/:name(/:tab)`, regex-sniffed by ServiceUrlSync) AND every genuinely unknown
// URL. Only the former gets the shell; anything else gets a real 404 instead of silently
// rendering it.
function HomeShellRoute({ children }: { children: ReactNode }) {
  const location = useLocation();
  return isHomeShellPath(location.pathname) ? <>{children}</> : <NotFoundPage />;
}
                  ...
                  <Route
                    ...
                    element={
                      /* Full-width shell — adapt to the monitor, no fixed max-w cap.
                         ...
                         HomeShellRoute gates this: only /home and /service/:name(/:tab) get the
                         shell below — every other unmatched URL gets a real 404 instead. */
                      <HomeShellRoute>
                        <main className="w-full app-shell-gutter py-4">
