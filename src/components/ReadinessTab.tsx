          ...
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              <div
                ...
                style={{ color: getReadinessColor(checklist.readiness_percent) }}
              >
                {checklist.readiness_percent}%
              </div>
              <div className="text-xs text-[var(--color-text-muted)] mt-1">
                {checklist.not_applicable_items > 0 ? (
                  <>{checklist.completed_items} of {checklist.total_items - checklist.not_applicable_items} applicable items</>
                ) : (
  ...
  return (
    <div
      className={cn(
        ...
        item.blocking && item.status !== 'done' && "border-[rgba(248,113,113,0.2)] bg-[rgba(248,113,113,0.05)]"
      )}
...
function VenueCoverageCard() {
  ...
  const cliCommand = 'python -m unified_trading_deployment.cli data-status -s instruments-service --start-date 2026-01-01 --end-date 2026-01-31 --check-venues'
    ...
    <Card className="border-[rgba(96,165,250,0.3)] bg-[rgba(96,165,250,0.05)]">
      <CardHeader className="pb-2">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs">CLI Only</Badge>
        </div>
        <CardDescription>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg bg-[var(--color-bg-tertiary)] border border-[var(--color-border-subtle)]">
          ...
          <div className="text-xs text-[var(--color-text-secondary)]">
            ...
            <p>
              While deployments now shard by <span className="font-mono">category × venue × date</span> (one
              shard per venue+date), venue coverage validation reads the <span className="font-mono">venue</span> column
              from parquet files to verify all expected venues are present within each file. This validates
              file <em>content</em>, not just file existence.
            </p>
            <p className="mt-2">
              This catches API adapter failures (rate limits, auth errors, empty responses) where a venue's
              API returned no data—issues that file existence checks would miss.
            </p>
          </div>
        </div>
        ...
        <div className="text-xs text-[var(--color-text-muted)]">
          ...
          <ul className="mt-1 space-y-1 ml-4 list-disc">
            <li>For each date, reads only the <code className="font-mono">venue</code> column (efficient)</li>
            <li>Compares found venues against expected venues from <code className="font-mono">expected_start_dates.yaml</code></li>
