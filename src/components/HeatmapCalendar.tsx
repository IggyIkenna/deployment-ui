  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 text-xs text-[var(--color-text-muted)]">
        ...
        <div className="flex items-center gap-1">
          <div
            ...
            style={{ backgroundColor: 'var(--color-accent-green)', opacity: 0.9 }}
          />
          <span>Complete</span>
        </div>
        <div className="flex items-center gap-1">
          <div
            className="w-3 h-3 rounded-sm"
            style={{ backgroundColor: 'var(--color-accent-amber)', opacity: 0.7 }}
          />
          <span>Partial</span>
        </div>
        <div className="flex items-center gap-1">
      ...
      <div className="flex flex-wrap gap-6">
        {calendarData.map((monthData: { year: number; month: number; weeks: (DayData | null)[][] }) => (
          <div key={`${monthData.year}-${monthData.month}`} className="flex-shrink-0">
            {/* Month header */}
            ...
            <div className="space-y-1">
              {monthData.weeks.map((week: (DayData | null)[], weekIdx: number) => (
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected && "ring-2 ring-[var(--color-accent-cyan)]"
                        )}
                        style={{
                          backgroundColor: getStatusColor(day.status, day.coverage),
                          opacity: getStatusOpacity(day.status),
                        }}
                        title={day.tooltip || `${day.date}: ${day.status}${day.coverage !== undefined ? ` (${day.coverage}%)` : ''}`}
                      >
  ...
  while (currentDate <= end) {
    ...
    if (isFuture) {
      ...
      if (!hasData) {
        ...
        const coverage = Math.round((categoriesWithData / totalCategories) * 100)
        result.push({
          date: dateStr,
          status: 'partial',
          coverage,
