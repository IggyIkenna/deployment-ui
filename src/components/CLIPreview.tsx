...
function buildCLICommand(
  ...
  if (formValues.extra_args && formValues.extra_args.trim()) {
    ...
    const extraArgs = quoted.split('--').filter(arg => arg.trim()).map(arg => `--${arg}`)
    args.push(...extraArgs)
  }
...
export function ContainerCommandPreview({
  ...
  const containerFlags: Record<string, string[]> = {
    ...
    'position-balance-monitor-service': ['--operation', 'monitor', '--mode', 'batch'],
  ...
  for (const dim of dimensions.dimensions) {
    ...
    } else if (dim.name === 'category' && serviceName === 'market-data-processing-service') {
