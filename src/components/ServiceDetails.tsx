  ...
  return (
    <div className="space-y-4">
      ...
      <Tabs defaultValue="dependencies" className="w-full">
        <TabsList variant="pill" className={`grid w-full ${isInfrastructure ? "grid-cols-1" : "grid-cols-2"}`}>
...
// Exported for the partial-payload regression test (ServiceDetails.test.tsx) — guards that a
// missing optional array never white-screens the app.
export function DependenciesPanel({ dependencies, currentService }: DependenciesPanelProps) {
  if (!dependencies) {
    return (
      <Card>
  ...
  // Defensive: a partial / late / raced dependencies payload (or schema drift) must render
  // empty, NEVER white-screen the whole app through the root ErrorBoundary. A missing optional
  // array here crashed the `.length`/`.map` reads below (intermittent — fixed 2026-06-11).
  const upstream = dependencies.upstream ?? [];
  const downstreamDependents = dependencies.downstream_dependents ?? [];
  const outputs = dependencies.outputs ?? [];
    ...
    <div className="space-y-4">
      ...
      {dependencies.dag && (
        <DependencyDag
          ...
          upstream={upstream.map((u) => u.service)}
        ...
        <CardContent>
          {downstreamDependents.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {downstreamDependents.map((service) => (
                <span
                  key={service}
...
export function DependencyDag({ dag, currentService, upstream, downstream }: DependencyDagProps) {
