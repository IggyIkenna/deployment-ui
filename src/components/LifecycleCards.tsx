...
/**
 * Loads new-listings/upcoming-expiries rows for a given threshold. The `kind`
 * discriminator (a stable literal, not a function prop) avoids re-fetch loops
 * from unstable callback identities across parent re-renders.
 */
function useLifecycleRows(kind: LifecycleKind, threshold: number) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data =
        kind === "new-listings"
          ? await fetchNewListings({ max_age_days: threshold })
          : await fetchUpcomingExpiries({ within_days: threshold });
      setRows(data);
    } catch (e) {
...
function LifecycleCard({
  ...
  const { rows, loading, error, reload } = useLifecycleRows(kind, threshold);
