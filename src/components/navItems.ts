...
type NavItem = {
  ...
  desc: string;
  /** Compact label for the always-visible cockpit bar (the flat mobile list uses `label`). */
  short?: string;
};
 ...
 * TopNavBar and the mobile hamburger list (Header) so the two never drift. The top-left
 * dropdown this SSOT also used to drive was DELETED 2026-07-28 (RULED — the bar is reachable
 * on every route, the dropdown's earlier cockpit placement was not; see
...
/** The 16 canonical entries, flat + in group order. Drives the always-visible cockpit bar. */
...
/**
 * Is `to` the screen currently on display? Drives the always-visible top bar's highlight rule.
 * Plain path match, with prefix matching for deeper routes (e.g. /deployments/:name keeps the
 * Deployments entry lit).
 */
...
/** Flattened {to,label} list for the mobile hamburger menu (same source as the top bar). */
