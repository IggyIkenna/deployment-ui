...
describe("formatEventDrivenCoverageLabel", () => {
  it("renders both numbers + empty-rate for PREDICTION (Polymarket) row", () => {
    ...
    expect(label.text).toBe("100.0% attempted · 23.4% captured (77% empty)");
    expect(label.tooltip).toContain("event-driven");
    expect(label.tooltip).toContain("fraction of known underlyings");
  });

  it("omits empty-rate when estimate is null (edge case)", () => {
    const label = formatEventDrivenCoverageLabel(100.0, 100.0, null);
    expect(label.text).toBe("100.0% attempted · 100.0% captured");
    // Still explains the two axes in the tooltip.
  ...
  it("handles undefined inputs without throwing", () => {
    ...
    // 0.0% / 0.0% with no empty-rate suffix
    expect(label.text).toBe("0.0% attempted · 0.0% captured");
  });

  it("rounds empty-rate to whole %", () => {
    // 0.834 → 83% empty (rounded)
    const label = formatEventDrivenCoverageLabel(100.0, 16.6, 0.834);
    expect(label.text).toBe("100.0% attempted · 16.6% captured (83% empty)");
  });
});
