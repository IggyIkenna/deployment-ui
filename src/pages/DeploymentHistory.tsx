...
import { fetchDeploymentHistory, rollbackDeployment } from '../api/deploymentApi'
...
export function DeploymentHistory() {
  ...
  if (loading) return <div style={{ padding: '24px', color: '#6B7280', fontFamily: 'monospace' }}>Loading deployment history...</div>
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'monospace' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <button
          onClick={load}
          style={{ padding: '6px 12px', background: '#1F2937', color: '#E5E7EB', border: '1px solid #374151', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}
        >
          Refresh
        </button>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #333' }}>
              ...
              <th style={{ textAlign: 'left', padding: '8px' }}>Triggered At</th>
          ...
          <tbody>
            {jobs.map((job) => (
              <tr key={job.job_id} style={{ borderBottom: '1px solid #222' }}>
                <td style={{ padding: '8px', fontWeight: 600 }}>{job.service_name}</td>
                <td style={{ padding: '8px', color: '#9CA3AF' }}>{job.version}</td>
                <td style={{ padding: '8px', color: '#9CA3AF' }}>{job.environment}</td>
                <td style={{ padding: '8px' }}>
                  <span style={{ color: STATUS_COLORS[job.status], fontWeight: 600 }}>{job.status}</span>
                </td>
                <td style={{ padding: '8px', color: '#9CA3AF', fontSize: '12px' }}>{job.triggered_by}</td>
                <td style={{ padding: '8px', color: '#9CA3AF', fontSize: '12px' }}>
                  {new Date(job.triggered_at).toLocaleString()}
                </td>
                <td style={{ padding: '8px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                  {job.logs_url && (
                    <a href={job.logs_url} target="_blank" rel="noopener noreferrer"
                  ...
                  {job.status === 'SUCCESS' && (
                    <button
                      ...
                      style={{
                        padding: '3px 8px',
                        background: rollingBack === job.job_id ? '#374151' : '#7F1D1D',
                        color: '#FCA5A5',
                        border: 'none',
                        borderRadius: '4px',
                        cursor: rollingBack === job.job_id ? 'not-allowed' : 'pointer',
                        fontSize: '11px',
                      }}
                    >
                      {rollingBack === job.job_id ? 'Rolling back...' : 'Rollback'}
                    </button>
                  )}
                </td>
              </tr>
