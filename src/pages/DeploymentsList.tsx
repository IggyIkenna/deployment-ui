...
export function DeploymentsList() {
  ...
  if (loading) return <div style={{ padding: '24px', color: '#6B7280', fontFamily: 'monospace' }}>Loading services...</div>
    ...
    <div style={{ padding: '16px', fontFamily: 'monospace' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          <span style={{ color: healthy === total ? '#16A34A' : '#D97706', fontSize: '14px' }}>
          ...
          <Link
            to="/deploy"
            style={{ padding: '6px 12px', background: '#2563EB', color: 'white', borderRadius: '4px', textDecoration: 'none', fontSize: '13px' }}
          >
            + Deploy
          </Link>
        </div>
      ...
      {services.length === 0 ? (
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {services.map((svc) => (
              <tr key={svc.service_id} style={{ borderBottom: '1px solid #222' }}>
                ...
                <td style={{ padding: '8px', color: '#9CA3AF' }}>{svc.current_version}</td>
                <td style={{ padding: '8px', color: '#9CA3AF' }}>{svc.environment}</td>
                <td style={{ padding: '8px', textAlign: 'right' }}>
                  <span style={{ color: svc.replicas_ready < svc.replicas_total ? '#D97706' : '#16A34A' }}>
