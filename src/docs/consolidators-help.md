...
- **backlog** — `pending / total`: `total` is the **same count as fed by** above (how many per-VM shard files currently exist for this consolidator); `pending` is how many of those were written _after_ the index's last merge, i.e. not yet folded in. `pending == total` (e.g. `2/2`) means every current writer is unmerged — the whole fan-in is behind. `pending < total` (e.g. `1/3`) means only some writers are behind while the rest are already reflected in the index — a milder, partial-straggler shape, not a fully-stuck merge. When there's a backlog, a second line shows **oldest** — the age of the oldest un-absorbed shard, i.e. how long the merge has been behind. It turns **red** once that wait exceeds the budget (the merge has been stuck that long).
...
Underneath it, **triggers every …** is the actual Cloud Scheduler cron that fires this consolidator's Cloud Run job — declared config (projected from terraform), not a live GCP read, and **not the same thing** as the `index age` budget above: every consolidator in this estate is currently scheduled every 1 minute regardless of kind, while the staleness budget genuinely varies (2m live-tick / 1h slow-merge / 24h batch) because it's judged against each job's real production cadence, not its trigger frequency. Shown even for a **not reporting** consolidator — the schedule still fires it on time; it just isn't confirmed to be doing anything yet.
