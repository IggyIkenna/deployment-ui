...
describe('Type contracts', () => {
  ...
  it('DeploymentStatus union covers all valid statuses', () => {
    const statuses: DeploymentStatus[] = [
      'pending', 'running', 'completed', 'completed_pending_delete',
      'completed_with_errors', 'completed_with_warnings', 'clean',
  ...
  it('ServiceHealth union covers all states', () => {
    const states: ServiceHealth[] = ['HEALTHY', 'DEGRADED', 'UNHEALTHY', 'UNKNOWN']
    expect(states).toHaveLength(4)
  })

  it('DeployJobStatus union covers all states', () => {
    const statuses: DeployJobStatus[] = ['QUEUED', 'RUNNING', 'SUCCESS', 'FAILED', 'CANCELLED']
