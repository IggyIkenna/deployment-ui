...
describe('TURBO_MODE_SERVICES', async () => {
  const { TURBO_MODE_SERVICES, TURBO_SUB_DIMENSION_SERVICES, UPSTREAM_CHECK_SERVICES } = await import('../../src/api/client')
  ...
  it('has sub-dimension mapping for turbo services', () => {
    ...
    expect(TURBO_SUB_DIMENSION_SERVICES['market-tick-data-handler']).toBe('data_type')
...
describe('deploymentApi', async () => {
  ...
  it('fetchServices calls correct endpoint', async () => {
    const mockServices = [{ service_id: 'svc-1', name: 'test', environment: 'dev', current_version: '1.0', health: 'HEALTHY', last_deployed_at: '2026-01-01', replicas_ready: 1, replicas_total: 1 }]
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/api/v1/services'))
  })
  ...
  it('triggerDeploy sends POST with JSON body', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ job_id: 'job-1', ...params, status: 'QUEUED', triggered_at: '2026-01-01', completed_at: null, logs_url: null, triggered_by: 'user', service_name: 'test' }),
    }))
